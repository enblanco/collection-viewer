...
consumer_key <- "wIGJIKhfUadrNVcPzlxq"
consumer_secret <-"qsYVEwhbESURxRFGDzhuJjAYbzANLChL"

#create unique user agent id
user_agent <-  'discollection_viewer'
...
ui <- navbarPage("Discogs collection viewer",
   ...
   fluidRow(
     ...
     column(3,
      wellPanel(
        textInput("username", label="User", value = "", width = NULL, placeholder = "Enter Discogs username"),
        actionButton("go", "Submit")),
    # bit with the years slider and x-axis var selector
      wellPanel(
#        selectInput("xvar", "What to look at?", c("Labels", "Artists", "Record formats",
#                                             "Release formats", "Year of issue", "Month bought", "Year bought"),
#                    selected = "Labels"),
        radioButtons("xvar", "Explore:",
                 c("Labels", "Artists", "Record formats", "Release formats", "Year of issue", "Collection by Month", "Collection by Year")),
        br(),
        sliderInput("year", "Choose digging years", min=2000, max=as.integer(format(Sys.Date(), "%Y")),
...
server <- function(input, output, session) {
  ...
  data <- eventReactive(input$go, {
    req(input$username)
    withProgress(message = 'Searching Collection...',
  ...
  output$plot <- renderHighchart ({
    ...
    if (input$xvar == "Labels") {
      ...
      hc_title(text = paste0("Your most collected labels, ", input$year[1], "-", input$year[2])) %>% 
      ...
      data_filtered() %>%
        ...
        hc_title(text = paste0("Your most collected artists, ", input$year[1], "-", input$year[2])) %>% 
      ...
      data_filtered() %>%
        summarise(`12"` = sum(`12"` == "12\"", na.rm=TRUE),
                 ...
                 `10"` = sum(format_name == "10\"", na.rm=TRUE),
                 `7"` = sum(format_name == "7\"", na.rm=TRUE),
        ...
        hc_title(text = paste0("Most collected record formats, ", input$year[1], "-", input$year[2])) %>% 
      ...
      data_filtered() %>%
        ...
        hc_title(text = paste0("Your most collected release formats, ", input$year[1], "-", input$year[2])) %>% 
    ...
    else if (input$xvar == "Collection by Month") {
      hchart(data_month(), "line", hcaes(x = month, y = month_rec_rate), name="Records", color="#252525") %>%
        hc_title(text = paste0("Average # of records collected by month, ", input$year[1], "-", input$year[2])) %>% 
        hc_xAxis(title = list(text = "Month")) %>%
        hc_yAxis(allowDecimals=FALSE, title = list(text = "Average no. of records bought")) %>%
        hc_add_theme(hc_theme_smpl())
    }
    else if (input$xvar == "Collection by Year") {
      ...
      data_filtered() %>%
        ...
        hchart("line", hcaes(x = as.factor(`lubridate::year(date_added)`), y = `Average no. of records bought per month`), 
               name="Records collected per year", color="#252525") %>%
        hc_title(text = paste0("# of Records, ", input$year[1], "-", input$year[2])) %>% 
        hc_xAxis(title = list(text = "Year")) %>%
        hc_yAxis(allowDecimals=FALSE, title = list(text = "Average no. of records collected per year")) %>%
        hc_add_theme(hc_theme_smpl())
    }
  })
